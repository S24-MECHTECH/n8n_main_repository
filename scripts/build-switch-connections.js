/**
 * 🔗 BUILD SWITCH ERROR HANDLER CONNECTIONS
 * Baut Connections für Switch-basierte Error Handler
 * Positioniert Nodes richtig basierend auf Update-Nodes
 */
...
async function buildConnections() {
  console.log('\n' + '='.repeat(80));
  console.log('🔗 BUILD SWITCH ERROR HANDLER CONNECTIONS');
  ...
  try {
    ...
    // 2. Für jeden Strang: Positioniere Switch-Node und baue Connections
    console.log('🔗 Baue Connections und positioniere Nodes...\n');
    ...
    for (const strand of strandDefinitions) {
      ...
      const errorHandlerNode = nodes.find(n => n.name === `Error Handler ${strand.name}`);
      ...
      if (!errorHandlerNode) {
        console.log(`   ⚠️  Error Handler ${strand.name} nicht gefunden - übersprungen\n`);
        continue;
      }
      
      // Positioniere Switch-Node rechts neben Update-Node
      if (updateNode.position) {
        errorHandlerNode.position = [
          updateNode.position[0] + 300,
          updateNode.position[1]
        ];
        console.log(`   ✅ Error Handler ${strand.name} positioniert`);
        totalPositions++;
      }
      
      // Connection: Update Node → Error Handler Switch
    ...
    // 4. REPORT
    console.log('📊 REPORT\n');
    console.log('✅ SWITCH ERROR HANDLER CONNECTIONS ERSTELLT');
    console.log(`   ✅ ${totalConnections} Connection(s) hinzugefügt`);
    console.log(`   ✅ ${totalPositions} Node(s) positioniert`);
    console.log(`   ✅ 6 Stränge konfiguriert`);
    console.log('\n   💡 Switch-Nodes haben 4 Outputs: RETRY, REROUTE, SKIP, ALERT');
    console.log('   💡 Diese können jetzt im n8n UI grafisch verbunden werden!\n');
    console.log('='.repeat(80) + '\n');
