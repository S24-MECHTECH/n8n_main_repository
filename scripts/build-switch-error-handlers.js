/**
 * 🔀 BUILD SWITCH-BASED ERROR HANDLERS
 * Erstellt Switch Nodes statt Code Nodes für Error Handling
 * GRAFISCH sichtbar!
 */
...
const strandDefinitions = [
  { name: 'Adult', updateNode: 'Update Product Adult Flag', baseX: 1000, baseY: 600 },
  { name: 'Images', updateNode: 'Update Product Images', baseX: 1000, baseY: 800 },
  { name: 'Text', updateNode: 'Update Product Text', baseX: 1000, baseY: 1000 },
  { name: 'Quality', updateNode: 'Update Merchant Settings', baseX: 1000, baseY: 1200 },
  { name: 'Country', updateNode: 'Update Country Feeds', baseX: 1000, baseY: 1400 },
  { name: 'GTN/EAN', updateNode: 'Update GTN/EAN', baseX: 1000, baseY: 1600 }
];

function createSwitchNode(strand, updateNodeId) {
  return {
    ...
    typeVersion: 3,
    position: [updateNodeId ? updateNodeId.position[0] + 400 : strand.baseX + 400, strand.baseY],
    parameters: {
      mode: 'rules',
      rules: {
        values: [
...
async function buildSwitchHandlers() {
  console.log('\n' + '='.repeat(80));
  console.log('🔀 BUILD SWITCH-BASED ERROR HANDLERS');
  ...
  try {
    ...
    // 2. Für jeden Strang Switch Node erstellen
    ...
    for (const strand of strandDefinitions) {
      ...
      const existingSwitch = nodes.find(n => n.name === `Error Handler ${strand.name}`);
      if (existingSwitch) {
    ...
    console.log(`   ✅ ${addedCount} neue Switch Node(s) hinzugefügt`);
    console.log(`   ✅ 6 Stränge konfiguriert`);
    console.log('\n   💡 Nächster Schritt: Connections bauen!');
    console.log('   💡 Switch Nodes sind GRAFISCH sichtbar!\n');
    console.log('='.repeat(80) + '\n');
...
module.exports = { buildSwitchHandlers };
